...
namespace Bannerlord.UIEditor.Core
{
    public interface ISettingsManager
    {
        ...
        /// </summary>
        T? GetSetting<T>(string _categoryName, string _name, T? _defaultValue);
        ...
        /// <exception cref="InvalidCastException"> <see cref="InvalidCastException"/>: Thrown if the setting exists and can't cast the value to <typeparam name="T">T</typeparam></exception>
        /// </summary>
        T? GetSetting<T>(string _categoryName, string _name);

        /// <summary>
        /// If the setting exists, updates its value, else creates the setting and sets its value.<br/>
        /// <exception cref="InvalidCastException"> <see cref="InvalidCastException"/>: Thrown if the setting exists and can't cast the value to <typeparam name="T">T</typeparam></exception>
        /// </summary>
        void SetSetting<T>(string _categoryName, string _name, T? _setting);

        ISettingCategory GetSettingCategory(string _categoryName);

        bool SettingCategoryExists(string _categoryName);
    }
}
